...
public class AStarDebug : MonoBehaviour {
        ...
        if (Input.GetMouseButtonDown(1))
        {
            ...
            if (hit.collider != null)
            {
                ...
                if (tmp != null)
                {
                    ...
                    {
                        goal = tmp;
                        CreateDebugTile(goal.WorldPosition, new Color32(135, 255, 0, 255));
                    }
                }
            }
        }
    ...
    public void DebugPath(HashSet<Node> openList, HashSet<Node> closedList, Stack<Node> path)
    {
        ...
        foreach (Node node in closedList) //Colours all the tiles blue so we can see which tiles are in the closed list
        {
            if (node.TileRef != start && node.TileRef != goal)
            {
